 ...
 * @LastEditors: 胡佳婷
 * @LastEditTime: 2023-04-27 15:11:21
 * @Description: 快速开始
-->
<template>
...
### 5. 根据自身项目需求，选择是否需要集成用户权限体系，目前支持的包括不集成用户权限体系(none)，IAM用户(iam)，天翼云用户(ctyun)，cuted内置用户(local)三种，具体配置请参考[用户权限](/docs/userperm)
#### 5-1. 如果用户类型是天翼云用户，可以选择是否集成IT埋点，默认是集成
#### 5-2. 如果是IAM用户，可以输入项目代码Domain，默认是'cdn'，可后续修改
### 6. 选择主题色，目前默认为白-蓝(default)，已支持黑-蓝(dark-blue)
### 7. 进入项目目录，试用\`cuted add <PageName> [optonal NewPageName]\` 生成demo页
### 8. [optional 安装依赖]，执行\`npm run serve\`(iam用户会执行npm run serve:iam，ctyun用户会执行npm run serve:ctyun，local或none用户会执行npm run serve:local)，启动项目即可
### 9. [备选] 若您不需要安装全局依赖，可以跳过第二步，并使用\`npx fe-cli create<projectName>\`和\`npx cute-design-tools add\`的方式使用

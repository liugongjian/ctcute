 ...
 * @LastEditTime: 2023-04-03 18:11:38
 * @Description: 基础表格
-->
<template>
...
export default class extends Vue {
  ...
  // 搜索信息
  private conditions: ProTable8.Conditions = {
    host: '',
    name: '',
  }
  ...
  // 表格数据
  private tableData: ProTable8.Host[] = null

  // 头部高度+面包屑高度+表格tool和表格高度+分页器高度+偏移量(两行表格的高度)
  // private tableOtherHeight = 50 + 66 + 34 + 24 + 44
  ...
  private async getTable() {
    try {
      this.loading = true
      // 分页信息和搜索条件
      const params: ProTable8.TableParams = {
   ...
   * 使用字典格式化实例状态
   @param data {ProTable8.Host} 表格行对象
 */
  private statusFormatter(data: ProTable8.Host) {
